...
**Sifra** — A secure, user-owned password vault evolving into a cross-platform, zero-knowledge digital vault and personal security platform.
...
Who this is for and what they are trying to get done. Take the roles from your own stories — they are written "As a <role>, I want …". Roles in your plan: user.
...
- **REQ-012** — The system must separate cloud-storage authentication from vault authentication.
...
- **r0** Minimum Viable Vault Core — 4 stories
- **r1** Google Drive Synchronization — 6 stories
- **r2** Extended Synchronization Providers — 1 story
- **r3** Device Management and Browser Integration — 2 stories
- **r4** Security Insights and Secure Sharing — 2 stories
...
- **User** — owns STORY-001, STORY-002, STORY-003, STORY-004, STORY-005, STORY-006, STORY-007, STORY-008, STORY-009, STORY-010, STORY-011, STORY-012, STORY-013, STORY-014
- **System** — owns STORY-015
...
- **REQ-001** (FUNC, must) — The system must allow a user to create a vault with a master password.
- **REQ-002** (FUNC, must) — The system must generate a one-time recovery key during vault creation.
- **REQ-003** (FUNC, must) — The system must support adding, viewing, editing, deleting, and searching login credentials.
- **REQ-004** (FUNC, must) — The system must generate strong passwords for user credentials.
- **REQ-005** (FUNC, must) — The system must allow manual copying of usernames and passwords.
- **REQ-006** (FUNC, must) — The system must lock and unlock the vault, rejecting incorrect passwords.
- **REQ-007** (FUNC, must) — The system must persist encrypted vault data locally without data loss on close/reopen.
- **REQ-008** (FUNC, must) — The system must allow changing the master password without re-encrypting all secrets.
- **REQ-009** (FUNC, must) — The system must support vault recovery using the recovery key to establish a new master password.
- **REQ-010** (FUNC, must) — The system must use Google Drive for encrypted cross-device synchronization.
- **REQ-011** (FUNC, should) — The system must use Microsoft OneDrive and Dropbox for encrypted cross-device synchronization after Google Drive.
- **REQ-012** (SAFE, must) — The system must separate cloud-storage authentication from vault authentication.
- **REQ-013** (CONSTRAINT, must) — The system must use established cryptographic libraries and primitives for encryption.
- **REQ-014** (FUNC, must) — The system must support local-first operation with offline usability.
- **REQ-015** (FUNC, should) — The system must support device identity and revocation.
- **REQ-016** (FUNC, should) — The system must support browser integration for credential discovery and autofill.
- **REQ-017** (FUNC, should) — The system must provide secure sharing of credentials and vaults.
- **REQ-018** (FUNC, should) — The system must support password health analysis and breach awareness.

## Your stories, in build order
**r0 · Minimum Viable Vault Core**
...
**r1 · Google Drive Synchronization**
