...
public sealed class PCloudCloudBlobStoreTests
{
    private static (PCloudCloudBlobStore Store, FakePCloudApiClient ApiClient) CreateConnectedStore()
    {
        var apiClient = new FakePCloudApiClient();
        var auth = new LocalFakeCloudAuthProvider();
        auth.SignIn("someone@example.com");
        return (new PCloudCloudBlobStore(apiClient, auth), apiClient);
    }
    ...
    public void Exists_WhenNotConnected_ThrowsSyncUnavailable()
    {
        ...
        var store = new PCloudCloudBlobStore(apiClient, auth);
    ...
    public void Upload_WhenTheApiClientFailsForNetworkReasons_PropagatesThePCloudException()
    {
        ...
        Assert.Throws<PCloudNetworkException>(() => store.Upload("att-1", [1, 2, 3]));
    }

    [Fact]
    public void AttachmentBlobSyncService_EndToEnd_TwoDevicesSharingOneFakePCloudApiClient_TransfersBlobs()
    {
        var apiClient = new FakePCloudApiClient();
        ...
        var deviceADirectory = Path.Combine(Path.GetTempPath(), "sifra-pcloud-blob-a-" + Guid.NewGuid());
        var deviceBDirectory = Path.Combine(Path.GetTempPath(), "sifra-pcloud-blob-b-" + Guid.NewGuid());
        try
        {
            ...
            var syncA = new AttachmentBlobSyncService(attachmentStoreA, new AttachmentTombstoneStore(deviceADirectory), new PCloudCloudBlobStore(apiClient, authA));
            syncA.SyncBlobs();
            ...
            var syncB = new AttachmentBlobSyncService(attachmentStoreB, new AttachmentTombstoneStore(deviceBDirectory), new PCloudCloudBlobStore(apiClient, authB));
            syncB.SyncBlobs();
