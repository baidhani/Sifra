...
public sealed class PCloudVaultEnvelopeCloudStoreTests
{
    private static VaultSyncEnvelope EmptyEnvelope() => new([], [], [], [], []);

    private static (PCloudVaultEnvelopeCloudStore Store, FakePCloudApiClient ApiClient) CreateConnectedStore()
    {
        var apiClient = new FakePCloudApiClient();
        var auth = new LocalFakeCloudAuthProvider();
        auth.SignIn("someone@example.com");
        return (new PCloudVaultEnvelopeCloudStore(apiClient, auth), apiClient);
    }
    ...
    public void Pull_WhenNotConnected_ThrowsSyncUnavailable()
    {
        ...
        var store = new PCloudVaultEnvelopeCloudStore(apiClient, auth);

        Assert.Throws<SyncUnavailableException>(() => store.Pull());
    }

    [Fact]
    public void Pull_WhenTheApiClientReportsUnauthorized_PropagatesThePCloudException()
    {
        var (store, apiClient) = CreateConnectedStore();
        apiClient.SimulateUnauthorized = true;

        Assert.Throws<PCloudUnauthorizedException>(() => store.Pull());
    }

    [Fact]
    public void Pull_WhenTheApiClientFailsForNetworkReasons_PropagatesThePCloudException()
    {
        var (store, apiClient) = CreateConnectedStore();
        apiClient.SimulateNetworkFailure = true;

        Assert.Throws<PCloudNetworkException>(() => store.Pull());
    }

    [Fact]
    public void VaultSyncService_EndToEnd_TwoDevicesSharingOneFakePCloudApiClient_SyncCorrectly()
    {
        // Proves PCloudVaultEnvelopeCloudStore genuinely satisfies
        ...
        var deviceADirectory = Path.Combine(Path.GetTempPath(), "sifra-pcloud-a-" + Guid.NewGuid());
        var deviceBDirectory = Path.Combine(Path.GetTempPath(), "sifra-pcloud-b-" + Guid.NewGuid());
        try
        {
            ...
            var syncA = new VaultSyncService(
                ...
                new PCloudVaultEnvelopeCloudStore(apiClient, authA));
            var syncB = new VaultSyncService(
                ...
                new PCloudVaultEnvelopeCloudStore(apiClient, authB));
